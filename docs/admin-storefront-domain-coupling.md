...
- [x] 5. Refactor [src/services/adminProductsService.js](src/services/adminProductsService.js) and [src/services/adminCategoriesService.js](src/services/adminCategoriesService.js) so they delegate to the new boundary while preserving the existing CRUD behavior.
- [x] 6. Update the admin managers in [src/components/Admin/Products/ProductsManager.js](src/components/Admin/Products/ProductsManager.js) and [src/components/Admin/Categories/CategoriesManager.js](src/components/Admin/Categories/CategoriesManager.js) to use the new boundary and keep the current user experience intact.
- [x] 7. Refactor [src/context/CatalogContext.js](src/context/CatalogContext.js) so the storefront continues to receive a stable catalog view model without coupling to admin mutation logic.

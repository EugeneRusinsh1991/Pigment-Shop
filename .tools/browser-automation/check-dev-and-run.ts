...
async function runAdminAndGuestSequential() {
  ...
  await runAutomation('.tools/browser-automation/run-admin-nav.ts');
  
  console.log('🔧 Running Guest automation...');
  await runAutomation('.tools/browser-automation/run-smoke-guest.ts');
...
async function main() {
  ...
  switch (command) {
    case 'admin-guest':
      ...
      break;
    case 'headless':
      await runAutomation('.tools/browser-automation/run-smoke-both-headless.ts');
      break;
    case 'playwright':

...
function resolveClassName(el) {
  if (typeof el.className !== 'string' || !el.className.trim()) return '';
  return `.${el.className.trim().split(/\s+/).join('.')}`;
}

function resolveText(el) {
  return (el.innerText || el.getAttribute('placeholder') || el.getAttribute('aria-label') || '').slice(0, 40).trim();
}

function buildTargetInfo(el) {
...
function getOrCreateOverlayBox() {
...
function getOrCreateOverlayBadge() {
...
function shouldIgnoreOverlayElement(el) {
  return !el || Boolean(el.closest('#manual-browser-inspector')) || el.id === '__mbi_live_box' || el.id === '__mbi_live_badge';
}

function updateOverlayPosition(overlayBox, overlayBadge, el, info) {
  const rect = el.getBoundingClientRect();
...
export default function ManualBrowserInspector() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (checkPlaywright()) clearInterval(interval);
    }, 500);
  ...
  useEffect(() => {
    ...
    overlayBox = getOrCreateOverlayBox();
    overlayBadge = getOrCreateOverlayBadge();

    const handleLiveHighlightMove = (e) => {
      const el = document.elementFromPoint(e.clientX, e.clientY);
      if (shouldIgnoreOverlayElement(el)) {
        overlayBox.style.display = 'none';
        overlayBadge.style.display = 'none';
        return;
      }
      updateOverlayPosition(overlayBox, overlayBadge, el, buildTargetInfo(el));
    };

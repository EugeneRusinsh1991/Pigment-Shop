...
function auditPerformance() {
  ...
  if (violations.length === 0) {
    ...
    report += `Found ${violations.length} performance issue(s) across ${fileCount} file(s):\n\n`;
    ...
    if (fileCount > 10) {
      ...
      console.log(`  -> Also generated compact file list: 10-performance-files.log`);
    } else {
      if (fs.existsSync(FILES_LOG_FILE)) { try { fs.unlinkSync(FILES_LOG_FILE); } catch (_) {} }
    }

    fs.writeFileSync(LOG_FILE, report);
    console.log(`[10 Performance Audit] Finished (${violations.length} issues) -> .docs/audits/audits/10-performance-violations.log`);
  }
}

...
function Invoke-RestoreBackup {
  ...
  $backupEntries = Get-ChildItem -Path $backupFolder | Select-Object -ExpandProperty Name

  Info "Removing current project files..."
  foreach ($item in $backupEntries) {
  ...
  foreach ($item in $backupEntries) {
    $srcItem = Join-Path $backupFolder $item
    $destItem = Join-Path $projectRoot $item

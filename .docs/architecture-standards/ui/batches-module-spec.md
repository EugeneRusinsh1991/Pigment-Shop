...
> [!NOTE]
> This engineering standard defines the architectural specification, directory layout, variant definitions, API contract, and design token integration patterns for the unified `Badge` status indicator primitive module across PigmentShop, implementing [architecture-standards/ui/reference-module-spec.md](file:///d:/Magazine/_PigmentShop/architecture-standards/ui/reference-module-spec.md).
...
Following the standard reference module architecture, the Badge primitive is structured under `src/components/ui/Badge/`:
...
All consumers must import badge elements exclusively from `src/components/ui/Badge/index.js`.

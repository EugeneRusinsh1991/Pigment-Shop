...
const useIconThemeColor = (color) => {
  let isDark = false;
  try {
    const theme = useTheme();
    isDark = theme?.isDark;
  } catch (e) {}
  return color || (isDark ? colors.textDark : colors.textLight);
};
...
export const CheckIcon = ({ color, size = 16, style, ...props }) => {
  const resolvedColor = useIconThemeColor(color);
  if (Platform.OS === 'web') {
    return (
      <svg width={size} height={size} viewBox={iconTokens.viewBox} fill={iconTokens.fillNone} stroke={resolvedColor} strokeWidth={iconTokens.strokeWidth.default} strokeLinecap="round" strokeLinejoin="round" style={getSvgStyle(style)} {...props}>
        <polyline points="20 6 9 17 4 12" />
      </svg>
    );
  }
  return <RNText style={getIconTextStyle(resolvedColor, size, style)} {...props}>✓</RNText>;
};
...
export const BackArrowIcon = ({ color, size = 16, style, ...props }) => {
  ...
  if (Platform.OS === 'web') {
    return (
      <svg width={size} height={size} viewBox={iconTokens.viewBox} fill={iconTokens.fillNone} stroke={resolvedColor} strokeWidth={iconTokens.strokeWidth.bold} strokeLinecap="round" strokeLinejoin="round" style={getSvgStyle(style)} {...props}>
...
export const EyeIcon = ({ color, size = 16, style, ...props }) => {
  ...
  return <RNText style={getIconTextStyle(resolvedColor, size, style)} {...props}>👁️</RNText>;
};

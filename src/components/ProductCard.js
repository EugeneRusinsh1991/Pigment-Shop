...
export default function ProductCard({ item, onPress, isDark, depth = 1, isFavorite, onToggleFavorite, overrideWidth }) {
  ...
  const { cardWidth: hookWidth, cardHeight, imgContainerHeight } = useCardDimensions(depth, true);
  ...
  return (
    <TouchableOpacity
      style={[
        styles.prodCard, 
        { width: cardWidth, minWidth: cardWidth, height: cardHeight, flex: 0, flexGrow: 0 }
      ]}
      onPress={onPress}
      activeOpacity={0.85}
    >

...
function removeOldDirectories(currentDir, directories) {
  console.log('🗑️  ШАГ 2: Очищаем старые файлы перед восстановлением (папка .tools защищена)...');
  directories.forEach((item) => {
    if (item === '.tools' || item.startsWith('_backup_before_restore_')) {
      return;
    }
...
function restoreFromBackup(backupPathArg) {
  ...
  const directories = BACKUP_ITEMS.filter((item) => {
    const backupItemPath = path.join(backupPath, item);
    return fs.existsSync(backupItemPath) && fs.statSync(backupItemPath).isDirectory();
  });
  const files = BACKUP_ITEMS.filter((item) => {
    const backupItemPath = path.join(backupPath, item);
    return fs.existsSync(backupItemPath) && fs.statSync(backupItemPath).isFile();
  });
